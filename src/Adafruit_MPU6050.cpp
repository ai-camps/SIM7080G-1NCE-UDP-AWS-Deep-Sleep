...
bool Adafruit_MPU6050::_init(int32_t sensor_id) {
  ...
  setGyroRange(MPU6050_GYR_RANGE_500_DEG);

  setAccelerometerRange(MPU6050_ACC_RANGE_2G); // already the default
...
void Adafruit_MPU6050::_read(void) {
  ...
  float accel_scale = 1;
  if (accel_range == MPU6050_ACC_RANGE_16G)
    accel_scale = 2048;
  if (accel_range == MPU6050_ACC_RANGE_8G)
    accel_scale = 4096;
  if (accel_range == MPU6050_ACC_RANGE_4G)
    accel_scale = 8192;
  if (accel_range == MPU6050_ACC_RANGE_2G)
    accel_scale = 16384;
  ...
  float gyro_scale = 1;
  if (gyro_range == MPU6050_GYR_RANGE_250_DEG)
    gyro_scale = 131;
  if (gyro_range == MPU6050_GYR_RANGE_500_DEG)
    gyro_scale = 65.5;
  if (gyro_range == MPU6050_GYR_RANGE_1000_DEG)
    gyro_scale = 32.8;
  if (gyro_range == MPU6050_GYR_RANGE_2000_DEG)
    gyro_scale = 16.4;
